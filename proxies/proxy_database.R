...
get_recent_timestamp <- function(pool, table) {
  column_check_query <- paste0("SELECT column_name FROM information_schema.columns WHERE table_name = '", table, "' AND column_name = 'insert_timestamp';")
  ...
  if (nrow(column_check_result) == 0) {
    return("insert_timestamp column does not exist in the table")
  }

  query <- paste0("SELECT \"insert_timestamp\" FROM \"", table, "\" ORDER BY \"insert_timestamp\" DESC LIMIT 1;")
  result <- DBI::dbGetQuery(pool, query)
  ...
  timestamp <- result$insert_timestamp[1]
...
get_throws_with_advanced_stats <- function(pool, gameID = NULL) {
  if (is.null(gameID)) {
    query <- glue_sql(
      "SELECT \"throws\".*, \"advanced_stats\".\"fv_thrower\" 
       FROM \"throws\" 
       LEFT JOIN \"advanced_stats\" 
       ON \"throws\".\"throw_id\" = \"advanced_stats\".\"throw_id\"",
      .con = pool
    )
  } else {
    query <- glue_sql(
...
get_game_prob <- function(pool, table1_name, table2_name, join_column, gameID) {
  ...
  query <- glue("SELECT game_quarter, graphing_win_prob, game_time_left, 
                        ...
                        \"ThrowerY\", home_team_score, away_team_score, 
...
get_player_passes_by_role <- function(pool, player_id, role = "Thrower") {
  ...
  query <- glue("
    SELECT t.*, a.* 
    FROM throws t
    LEFT JOIN advanced_stats a ON t.\"throw_id\" = a.\"throw_id\"
...
get_user_credentials <- function(pool) {
  query <- glue::glue_sql("
    SELECT *
    FROM users
  ", .con = pool)
...
user_exists <- function(pool, username) {
  sql <- glue::glue_sql("SELECT COUNT(*) FROM users WHERE username = {username}", .con = pool)
  res <- DBI::dbGetQuery(pool, sql)
  res[[1]] > 0
}

register_user <- function(pool, username, email, password) {
  # Check if username or email already exists
  user_check_sql <- glue::glue_sql(
    "SELECT COUNT(*) AS count FROM users WHERE username = {username} OR email = {email}",
    .con = pool
  )
  user_exists <- DBI::dbGetQuery(pool, user_check_sql)$count > 0
  if (user_exists) return(FALSE)

  # Hash password securely
  hashed_pw <- sodium::password_store(password)

  # Insert new user with email and permissions
  insert_sql <- glue::glue_sql(
    "INSERT INTO users (username, email, password, permissions)
     VALUES ({username}, {email}, {hashed_pw}, 'standard')",
    .con = pool
  )

  DBI::dbExecute(pool, insert_sql)
  TRUE
}


verify_user <- function(pool, username, password_attempt) {

  sql <- glue::glue_sql("SELECT password FROM users WHERE username = {username}", .con = pool)
  stored <- DBI::dbGetQuery(con, sql)
  if (nrow(stored) == 0) return(FALSE)  # user not found
  sodium::password_verify(stored$password[1], password_attempt)
}